...
$college = array(
  ...
  'Quatrieme' => array(
   ...
   'Nouveaux'=> array (
       array('Nom' => 'Leclercq', 'Prenom' => 'Cyriak'),
       array('Nom' => 'Caron', 'Prenom' => 'Gregory'),
       array('Nom' => 'Ducamp', 'Prenom' => 'Maxime'),
       )
);

echo '10.Les eleves du college : <br>';
//ajoutez votre code ici
foreach ($college as $class=> $valeur){
    echo $class .'<br/><br/>';
    foreach ($valeur as $Tab=>$name){
        echo $name['Nom']. ' , ' .$name['Prenom'].'<br/>';
    }
}
echo '<br><br>';
...
//ajoutez votre code ici
foreach ($videotheque as $movie=>$film){

        echo $film['nom'].'<br>'.$film['date'].'<br>'.$film['realisateur'].'<br>'.$film['acteurs'][0].'<br><br>';
